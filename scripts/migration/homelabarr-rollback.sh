...
# HomelabARR to DockServer Rollback Script
# This script helps users rollback from HomelabARR to DockServer if needed
...
find_backup() {
    ...
    local backups=($(ls -d "$backup_base"/dockserver-migration-* 2>/dev/null | sort -r))
...
restore_paths() {
    ...
    if [ -d "/opt/homelabarr" ] && [ ! -d "/opt/dockserver" ]; then
        print_info "Moving /opt/homelabarr back to /opt/dockserver..."
        mv "/opt/homelabarr" "/opt/dockserver"
    fi
    
    if [ -d "/opt/appdata/homelabarr" ] && [ ! -d "/opt/appdata/dockserver" ]; then
        print_info "Moving /opt/appdata/homelabarr back to /opt/appdata/dockserver..."
        mv "/opt/appdata/homelabarr" "/opt/appdata/dockserver"
    fi
...
restore_systemd() {
    ...
    for service in "${service_files[@]}"; do
        if [ -f "$service.bak" ]; then
            old_service="${service/homelabarr/dockserver}"
...
# Pull DockServer images
pull_images() {
    print_info "Pulling DockServer images..."
    
    local images=(
        "dockserver/dockserver:latest"
        "dockserver/dockserver-ui:latest"
    )
    ...
    print_success "DockServer images pulled"
}
...
verify_rollback() {
    ...
    # Check for DockServer paths
    if [ ! -d "/opt/dockserver" ]; then
        print_warning "/opt/dockserver directory not found"
        ((errors++))
    fi
...
main() {
    echo "========================================="
    echo "  HomelabARR to DockServer Rollback"
    echo "========================================="
    echo ""
    
    check_root
    
    print_warning "This script will rollback your HomelabARR installation to DockServer."
    ...
    print_info "  cd /opt/dockserver && docker-compose up -d"
    echo ""
