...
Autoscan integrates with Sonarr, Radarr, Lidarr and local filesystem monitoring to fetch changes in near real-time.
...
- HomelabARR CLI's Autoscan focuses on local NAS integration instead of cloud storage
- Direct filesystem monitoring eliminates dependency on external APIs  
...
- **Local Filesystem Monitoring**: Direct integration with local storage systems through Inotify triggers for real-time file change detection.
...
For optimal performance with local NAS storage, configure Autoscan with direct path mappings:

```yaml
triggers:
  # Local filesystem monitoring for NAS setups
  inotify:
    - priority: 5
      include:
        - ^/mnt/nas/media/
...
This configuration provides superior performance compared to cloud-based monitoring.
...
triggers:
  ...
  # Local NAS filesystem monitoring
  nas-monitor:
    priority: 5
    rewrite: # Local NAS to unified filesystem
      - from: ^/mnt/nas/
        to: /mnt/unionfs/
