...
app.get('/containers', conditionalAuth, async (req, res) => {
  try {
    ...
    if (!includeStats) {
      ...
      const containersWithBasicInfo = await Promise.all(
        containers.map(async (container) => {
          try {
            // Windows-compatible CLI-based container inspection
            const { execSync } = await import('child_process');
            const inspectResult = execSync(`powershell.exe -Command "docker inspect ${container.Id}"`, {
              encoding: 'utf8',
              maxBuffer: 10 * 1024 * 1024
            });
            
            const info = JSON.parse(inspectResult.trim())[0];
    ...
    const containersWithStats = await Promise.all(
      containers.map(async (container) => {
        try {
          // Windows-compatible CLI-based container inspection and stats
          const { execSync } = await import('child_process');
          const [statsResult, inspectResult] = await Promise.all([
            new Promise((resolve, reject) => {
              try {
                const statsOutput = execSync(`powershell.exe -Command "docker stats ${container.Id} --no-stream --format json"`, {
                  encoding: 'utf8',
                  maxBuffer: 10 * 1024 * 1024
                });
                resolve(JSON.parse(statsOutput.trim()));
              } catch (error) {
                // Stats failed, continue with default values
                resolve(null);
              }
            }),
            new Promise((resolve, reject) => {
              try {
                const inspectOutput = execSync(`powershell.exe -Command "docker inspect ${container.Id}"`, {
                  encoding: 'utf8',
                  maxBuffer: 10 * 1024 * 1024
                });
                resolve(JSON.parse(inspectOutput.trim())[0]);
              } catch (error) {
                reject(error);
              }
            })
          ...
          return {
            ...container,
            stats: {
              cpu: stats ? parseFloat(stats.CPUPerc || '0') : 0,
              memory: stats ? {
                usage: parseFloat(stats.MemUsage || '0'),
                limit: parseFloat(stats.MemLimit || '0'),
                percentage: parseFloat(stats.MemPerc || '0')
              } : { usage: 0, limit: 0, percentage: 0 },
              network: stats ? {
