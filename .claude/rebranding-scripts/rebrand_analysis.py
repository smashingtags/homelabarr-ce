...
def analyze_rebranding_scope():
    ...
    categories = {
        'docker_images': [],       # Docker image references (ghcr.io/dockserver/*)
        'file_paths': [],          # File system paths (/opt/dockserver/*)
        'binary_names': [],        # Binary/executable names (dockserver command)
    ...
    patterns = {
        r'ghcr\.io/dockserver/': 'docker_images',
        r'/opt/dockserver': 'file_paths',
        r'/usr/bin/dockserver': 'binary_names',
        r'/bin/dockserver': 'binary_names',
        r'docker-mod-\w+': 'docker_mods',
        r'Copyright.*dockserver': 'copyright',
        r'\bmrdoob\b': 'usernames',
        r'\bMrDoob\b': 'usernames',
        r'dockserver-local': 'network_names',
        r'--name=dockserver': 'container_names',
        r'# Docker.*dockserver': 'comments'
    }
    ...
    for root, dirs, files in os.walk('.'):
        ...
        for file in files:
            ...
            try:
                ...
                # Check for any dockserver references
                if re.search(r'\b(dockserver|mrdoob|MrDoob|Dockserver)\b', content, re.IGNORECASE):
...
def generate_replacement_mapping():
    ...
    return {
        ...
        'ghcr.io/dockserver/': 'ghcr.io/homelabarr/',  # or your new registry
        
        # File paths
        '/opt/dockserver': '/opt/homelabarr-cli',
        
        # Binary names
        '/usr/bin/dockserver': '/usr/bin/homelabarr',
        '/bin/dockserver': '/bin/homelabarr',
        'dockserver': 'homelabarr',
        
        # Network names
        'dockserver-local': 'homelabarr-local',
        
        # Container names
        '--name=dockserver': '--name=homelabarr',
