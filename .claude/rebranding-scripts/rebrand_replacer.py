...
class RebrandReplacer:
    def __init__(self, new_project_name="homelabarr-cli", new_username="homelabarr"):
        ...
        self.replacements = {
            ...
            r'ghcr\.io/dockserver/': f'ghcr.io/smashingtags/',
            
            # File paths
            r'/opt/dockserver\b': f'/opt/{new_project_name}',
            
            # Binary names and commands
            r'/usr/bin/dockserver\b': f'/usr/bin/{new_project_name}',
            r'/bin/dockserver\b': f'/bin/{new_project_name}',
            r'\bdockserver\b(?=\s|$|\.|\s)': new_project_name,  # Standalone dockserver
            
            # Network names
            r'dockserver-local': f'{new_project_name}-local',
            
            # Container names
            r'--name=dockserver\b': f'--name={new_project_name}',
            ...
            r'\bDockserver\b': new_project_name.title().replace('-', ''),
            r'\bDockServer\b': new_project_name.title().replace('-', ''),
            
            # Docker mod names (keep dockserver prefix in mod names for now)
...
def main():
    ...
    print(f"  Project name: dockserver -> {args.new_name}")
