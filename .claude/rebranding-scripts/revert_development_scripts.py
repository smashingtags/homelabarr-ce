...
def revert_development_scripts():
    ...
    dev_scripts_dir = Path("F:/Coding Projects/dockserver/.claude/development-scripts")
    backup_dir = Path("F:/Coding Projects/dockserver/dev_scripts_backup_" + 
        ...
        if any(keyword in script_file.name.lower() for keyword in ['convert', 'migrate', 'fix', 'bulk']):
            ...
            conversion_patterns = [
                (r'echo.*"Converting.*homelabarr-cli', 
                 lambda m: m.group(0).replace('homelabarr-cli', 'dockserver')),
