...
confluence_url: "https://your-instance.atlassian.net/wiki/spaces/DO/pages/7503955"
confluence_space: "DO"
category: "General"
created_date: ""
updated_date: ""
...
### The Evolution: PGBlitz → Homelabarr → HomelabARR

**The Complete Timeline:**1.**PGBlitz Era (2018-2020)**: The original unlimited Google Drive media server solution 2.**Homelabarr Fork (2020-2023)**: PGBlitz fork that continued after the original ceased development
3.**HomelabARR (2023-Present)**: Original platform that strategically integrated Homelabarr's CLI components
### The Real Story:

As a Software Development Project Manager by trade (not a coder initially), I was deeply involved in the PGBlitz community during the Google unlimited storage era. When PGBlitz development ceased, Homelabarr emerged as its spiritual successor. However, I envisioned something better - a modern web-based platform that would be accessible to non-technical users.
...
To accelerate the MVP, I made the strategic decision to integrate Homelabarr's proven Docker Compose templates rather than recreate them from scratch. This saved 6+ months of development time while allowing focus on the superior user experience.
...
### What Was Adapted from Homelabarr:
...
This document records all significant architectural decisions made during the development of HomelabARR and the integration of Homelabarr's CLI components. Each decision was carefully considered to create a sustainable, user-friendly, and maintainable self-hosted media server platform for the post-unlimited-storage era.
> 

**Note**: This is not a fork - it's an original platform that integrated useful components from Homelabarr while discarding its technical debt and outdated architecture.
## Decision Log

### 1. Integrate Homelabarr CLI Rather Than Build From Scratch

**Context:**- HomelabARR webapp and API were already built - Needed Docker Compose management system - Homelabarr had working YAML templates - Time to market was important for MVP

**Decision:**Adapt Homelabarr's CLI repository while maintaining HomelabARR's architecture.
...
**Context:**- PGBlitz/Homelabarr were built for unlimited Google Drive - Complex GDSA rotation to bypass 750GB/day upload limits - Google ended unlimited storage in 2022 - Most users now prefer local storage
...
**Consequences:**- ✅ Simpler setup process - ✅ Faster container startup - ✅ Works out-of-box with local storage - ⚠️ Users migrating from PGBlitz/Homelabarr need to adjust configs
### 3. Eliminate Mount Service Dependencies

**Context:**- Homelabarr required mount service to start before any media containers - Created cascading startup failures if mount had issues - Forced cloud storage on users who only wanted local storage
