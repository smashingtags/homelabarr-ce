# HomelabARR to HomelabARR Migration Guide

## Overview

This guide provides comprehensive instructions for migrating from HomelabARR to HomelabARR. The migration process has been designed to be as seamless as possible while preserving your data and configurations.
...
1. **Backup Your Data**: Ensure you have a complete backup of your HomelabARR installation
...
wget https://raw.githubusercontent.com/smashingtags/homelabarr-cli/main/scripts/migration/homelabarr-to-homelabarr.sh

# Make it executable
chmod +x homelabarr-to-homelabarr.sh

# Run the migration
sudo ./homelabarr-to-homelabarr.sh
```

The script will:
- Create a backup of your current configuration
- Stop HomelabARR containers
...
sudo mkdir -p /opt/backups/homelabarr-manual-$(date +%Y%m%d)

# Backup docker-compose files
sudo cp -r /opt/homelabarr /opt/backups/homelabarr-manual-$(date +%Y%m%d)/

# List appdata for reference
ls -la /opt/appdata > /opt/backups/homelabarr-manual-$(date +%Y%m%d)/appdata-list.txt
```

#### Step 2: Stop HomelabARR Containers

```bash
# Stop all HomelabARR containers
docker stop homelabarr homelabarr-ui homelabarr-autoscan homelabarr-mount

# Remove containers
docker rm homelabarr homelabarr-ui homelabarr-autoscan homelabarr-mount
```
...
```yaml
# Old
image: homelabarr/homelabarr:latest
...
sed -i 's/HOMELABARR_/HOMELABARR_/g' /opt/homelabarr/.env
sed -i 's/DS_/HL_/g' /opt/homelabarr/.env
```
...
```bash
# Move main directory
sudo mv /opt/homelabarr /opt/homelabarr

# Move appdata if exists
if [ -d "/opt/appdata/homelabarr" ]; then
    sudo mv /opt/appdata/homelabarr /opt/appdata/homelabarr
fi
```
...
sudo sed -i 's/homelabarr/homelabarr/g' /etc/systemd/system/homelabarr*.service

# Rename service files
sudo mv /etc/systemd/system/homelabarr.service /etc/systemd/system/homelabarr.service
...
5. Pull HomelabARR images
6. Start HomelabARR containers
...
- `homelabarr-legacy-base`: HomelabARR compatibility layer
...
- Custom scripts referencing HomelabARR paths need updating
- API endpoints may have changed
- Some HomelabARR-specific features may not be available
...
- Full HomelabARR compatibility maintained
...
- HomelabARR compatibility ends
- Full HomelabARR ecosystem only

## Conclusion

The migration from HomelabARR to HomelabARR represents a significant improvement in:
